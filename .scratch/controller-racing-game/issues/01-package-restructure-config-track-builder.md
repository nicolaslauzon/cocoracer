...
- [ ] Repo is a package; old experimental scripts removed; dependencies updated (add YAML parsing, web server, pytest; drop teleop/matplotlib deps)
- [ ] All game settings live in the one param file and load into typed config objects
- [ ] CLI parses `time-trial` and `race` with track and controller arguments and reports the resolved config and track geometry
- [ ] Track builder validates closure (turn angles sum to ±360°, endpoints close); a malformed spec fails loudly with a clear error
- [ ] Stadium track builds: resampled centerline, Frenet roundtrip (Cartesian → Frenet → Cartesian) returns the same point, occupancy grid marks exactly the cells beyond half-width from the centerline
- [ ] Track-seam tests pass: closure rejection, Frenet roundtrip, grid-vs-half-width
