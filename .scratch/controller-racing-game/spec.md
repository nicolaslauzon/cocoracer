...
27. As a developer, I want the disparity-extender baseline to extend each laser range edge across the car's width and steer toward the farthest point of the front sector, with speed limited by target distance and the current steering angle, so that it demonstrates a different, local-reactive driving style.
...
- **Baselines**: wall follow — pick the nearest wall in a forward sector, hold a target lateral distance via P/D on the heading error to an offset target point, speed = v_ref × (1 − k·|steering|) clamped. Disparity extender — over the front sector (±90°), extend each range edge (adjacent beams differing by a threshold) across the car's width, steer toward the farthest extended beam with a P/D law on its angle, speed from the distance to that target (full speed when far, a flat brake zone when close, linear in between) capped by the friction-limited speed for the commanded steering angle. Pure pursuit — ported from the existing experiment, tracks the centerline with speed-scaled lookahead. Every gain, offset, sector angle, and speed factor is a parameter in the param file.
...
- **Baseline semantics follow the F1TENTH course kit**: wall follow is the PID corridor-following of Lab 3; disparity extender is the reactive range-edge-extension obstacle avoidance of Lecture 5.
