...
- [ ] The engine records the pre-reset (x, y) at the tick a crash is registered, for wall hits and vehicle-to-vehicle hits alike
- [ ] The dynamic message's per-vehicle object gains `last_crash`: `{x, y}` or `null` before any crash
- [ ] The client stores each new crash mark at the reported crash position; marks still age out after half a lap of driving
- [ ] Engine/protocol tests: a scripted crash asserts `last_crash` equals the pre-reset pose; a fresh race serializes `null`
- [ ] Frontend structural test: the page references the `last_crash` field name
